...
This gate runs the attested `hms-agent.exe` artifact as a real Windows service on an ephemeral GitHub-hosted Windows runner.
...
The qualification harness fails closed unless all of these are proven with the packaged executable:
...
- package manifest, executable size/SHA-256 and Windows AMD64 PE shape are valid before install;
- a fresh machine-scope DPAPI device credential is created in the managed State directory;
- the production service-install script creates `HMSAgent` as `NT AUTHORITY\\LocalService`, enables unrestricted per-service SID and applies the production ACL contract;
...
- the production service-readiness contract passes;
- the packaged process publishes valid `/healthz` evidence reporting `NT SERVICE\\HMSAgent`, `non-admin`, the exact instance/workspace/version and canonical capability set;
