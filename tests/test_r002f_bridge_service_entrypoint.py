...
from hms_gpt_vps.bridge_service_runtime_config import BRIDGE_SERVICE_RUNTIME_SCHEMA_VERSION, BridgeServiceRuntimeConfig
...
def _config(tmp_path: Path) -> BridgeServiceRuntimeConfig:
    runtime_root=tmp_path/"runtime"; (runtime_root/"secrets").mkdir(parents=True); tls_root=tmp_path/"tls-private-key"
...
def test_runtime_factory_is_lazy_and_wraps_pairing_surface_after_build(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    config=_config(tmp_path); events=[]; base=object.__new__(BridgeProductionServiceRuntime); wrapped=object.__new__(BridgePairingSurfaceRuntime)
    monkeypatch.setattr(entry_module,"prove_hms_bridge_runtime_identity",lambda sid:events.append("identity") or {"process_sid":sid})
    monkeypatch.setattr(entry_module,"build_bridge_production_service_runtime",lambda runtime_config,verifier:events.append("build") or base)
    def load_config(): events.append("config"); return config
    def load_verifier(checked): assert checked is config; events.append("verifier"); return _Verifier()
    def wrap(inner,sid): assert inner is base and sid==_SERVICE_SID; events.append("wrap"); return wrapped
    factory=build_hms_bridge_runtime_factory(_SERVICE_SID,config_loader=load_config,verifier_loader=load_verifier,runtime_wrapper=wrap)
    assert events==[]; assert factory() is wrapped
    assert events==["identity","config","verifier","build","wrap"]

def test_service_entrypoint_passes_lazy_factory_before_config_read(monkeypatch: pytest.MonkeyPatch) -> None:
    events=[]
    def config_loader(): events.append("config"); raise AssertionError("SCM host must own identity-before-config")
    def fake_host(*,expected_service_sid,runtime_factory): events.append("host"); assert expected_service_sid==_SERVICE_SID and callable(runtime_factory)
    monkeypatch.setattr(entry_module,"run_hms_bridge_windows_service",fake_host)
    run_hms_bridge_service_entrypoint(sid_resolver=lambda:events.append("sid") or _SERVICE_SID,config_loader=config_loader,verifier_loader=lambda c:_Verifier(),runtime_wrapper=lambda i,s:pytest.fail("factory must stay lazy"))
...
def test_pairing_link_cli_uses_pid_pinned_ipc_and_prints_result(monkeypatch: pytest.MonkeyPatch,capsys: pytest.CaptureFixture[str]) -> None:
    import sys
    ...
    monkeypatch.setattr(ipc_module,"request_pairing_link_from_running_hms_bridge",lambda:PairingLinkIpcResult("pair-1","2026-08-26T01:02:03Z","https://bridge.example.test/pair/pair-1#token=abc"))
    monkeypatch.setattr(sys,"argv",["hms-bridge","pairing-link"]); assert cli_module.main()==0
    assert json.loads(capsys.readouterr().out)=={"pair_id":"pair-1","expires_at":"2026-08-26T01:02:03Z","pairing_link":"https://bridge.example.test/pair/pair-1#token=abc"}

def test_bridge_cli_has_no_config_or_pid_override() -> None:
    parser=build_parser()
    with pytest.raises(SystemExit): parser.parse_args(["service","--config","attacker.json"])
    with pytest.raises(SystemExit): parser.parse_args(["pairing-link","--pid","1234"])
    with pytest.raises(SystemExit): parser.parse_args(["pairing-link","--config","attacker.json"])
