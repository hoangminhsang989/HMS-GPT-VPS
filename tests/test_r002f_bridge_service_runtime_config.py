...
def _mapping(tmp_path: Path) -> dict[str, object]:
    ...
    (runtime_root / "secrets").mkdir(parents=True, exist_ok=True)
...
def test_config_converts_to_fixed_private_network_and_tunnel_authority(tmp_path: Path) -> None:
    ...
    assert runtime.secret_storage.root == Path(config.runtime_root) / "secrets" / "service-runtime"


def test_config_rejects_duplicate_json_key(tmp_path: Path) -> None:
    raw = _mapping(tmp_path)
    text = json.dumps(raw, separators=(",", ":"))
    duplicate = (f'{{"schema_version":{BRIDGE_SERVICE_RUNTIME_SCHEMA_VERSION},' + text[1:]).encode("utf-8")
    with pytest.raises(BridgeServiceRuntimeConfigError, match="duplicate Bridge runtime config key"):
        parse_bridge_service_runtime_config(duplicate)


def test_config_rejects_unknown_field(tmp_path: Path) -> None:
    raw = _mapping(tmp_path); raw["network_override"] = "0.0.0.0/0"
    with pytest.raises(BridgeServiceRuntimeConfigError, match="unknown=network_override"):
...
def test_config_rejects_invalid_pair_ttl_at_parse_boundary(tmp_path: Path) -> None:
    raw = _mapping(tmp_path); raw["pair_ttl_seconds"] = 30
    with pytest.raises(ValueError, match="pair_ttl_seconds"):
        parse_bridge_service_runtime_config(_bytes(raw))


def test_config_rejects_noncanonical_sha256(tmp_path: Path) -> None:
    raw = _mapping(tmp_path); raw["tls_certificate_der_sha256"] = "B" * 64
    with pytest.raises(BridgeServiceRuntimeConfigError, match="canonical lowercase SHA-256"):
        parse_bridge_service_runtime_config(_bytes(raw))


def test_config_requires_exact_tunnel_id(tmp_path: Path) -> None:
    raw = _mapping(tmp_path); del raw["tunnel_id"]
    with pytest.raises(BridgeServiceRuntimeConfigError, match="missing=tunnel_id"):
        parse_bridge_service_runtime_config(_bytes(raw))
    raw = _mapping(tmp_path); raw["tunnel_id"] = "tunnel_" + "G" * 32
    with pytest.raises(BridgeServiceRuntimeConfigError, match="tunnel_id"):
        parse_bridge_service_runtime_config(_bytes(raw))
