...
def validate_install_result(
    ...
    *,
    ...
    agent_root = _require_windows_path(
        payload["agent_root"], service.agent_root_path, f"{label} agent_root"
    )
    package_root = _require_windows_path(
        payload["package_root"], service.package_path, f"{label} package_root"
    )
    ...
    binary_path = _require_windows_path(
        payload["binary_path"], service.binary_path, f"{label} binary_path"
    )
    binary_sha256 = require_sha256_evidence(
        payload["binary_sha256"], f"{label} binary_sha256"
    )
    ...
    workspace = _require_windows_path(
        payload["workspace"], service.workspace_path, f"{label} workspace"
    )
    state_path = _require_windows_path(
        payload["state_path"], service.state_path, f"{label} state_path"
    )
...
def validate_native_scm_proof(proof: object) -> dict[str, object]:
    ...
    if (
        require_string_evidence(payload["qualification"], f"{label} qualification")
        != "native_windows_scm_packaged_agent"
    ):
    ...
    package_version = require_string_evidence(package["version"], "native SCM package version")
    ...
    package_file_count = require_int_evidence(
        package["file_count"], "native SCM package file_count", minimum=1
    )
    package_total_size = require_int_evidence(
        package["total_size"], "native SCM package total_size", minimum=1
    )
    package_sha256 = require_sha256_evidence(
        package["entrypoint_sha256"], "native SCM package entrypoint_sha256"
    )
    ...
    if (
        require_string_evidence(install["start_name"], "native SCM install start_name")
        != r"NT AUTHORITY\LocalService"
    ):
        raise RuntimeError("native SCM install account mismatch")
    if (
        require_string_evidence(
            install["service_sid_type"], "native SCM install service_sid_type"
        )
        != "UNRESTRICTED"
    ):
        raise RuntimeError("native SCM install SID type mismatch")
    if (
        require_int_evidence(
            install["package_file_count"],
            "native SCM install package_file_count",
            minimum=1,
        )
        != package_file_count
    ):
    ...
    if (
        require_int_evidence(
            install["package_total_size"],
            "native SCM install package_total_size",
            minimum=1,
        )
        != package_total_size
    ):
    ...
    if (
        require_sha256_evidence(
            install["binary_sha256"], "native SCM install binary_sha256"
        )
        != package_sha256
    ):
        raise RuntimeError("native SCM install/package entrypoint SHA-256 mismatch")
    require_sha256_evidence(
        install["runtime_config_sha256"], "native SCM install runtime_config_sha256"
    )
    ...
    if (
        require_int_evidence(
            readiness["package_file_count"],
            "native SCM readiness package_file_count",
            minimum=1,
        )
        != package_file_count
    ):
    ...
    if (
        require_int_evidence(
            readiness["package_total_size"],
            "native SCM readiness package_total_size",
            minimum=1,
        )
        != package_total_size
    ):
    ...
    validate_listener_result(payload["first_listener"], service_name="HMSAgent")
    validate_listener_result(payload["second_listener"], service_name="HMSAgent")

    first_epoch = require_int_evidence(
        payload["first_epoch"], "native SCM first_epoch", minimum=1
    )
    reconnect_epoch = require_int_evidence(
        payload["reconnect_epoch"], "native SCM reconnect_epoch", minimum=1
    )
    post_restart_epoch = require_int_evidence(
        payload["post_restart_epoch"], "native SCM post_restart_epoch", minimum=1
    )
    if not first_epoch < reconnect_epoch < post_restart_epoch:
        raise RuntimeError("native SCM epoch ordering mismatch")

    first_health = _require_object(payload["first_health"], "native SCM first_health")
    _require_exact_keys(
        first_health,
        frozenset(
            {
        ...
        "native SCM first_health",
    )
    if require_string_evidence(first_health["instance_id"], "native SCM first_health instance_id") != "ci-native-scm":
        raise RuntimeError("native SCM health instance identity mismatch")
    if require_string_evidence(first_health["agent_version"], "native SCM first_health agent_version") != package_version:
        raise RuntimeError("native SCM health/package version mismatch")
    if require_string_evidence(first_health["service_identity"], "native SCM first_health service_identity") != r"NT SERVICE\HMSAgent":
        raise RuntimeError("native SCM first health service identity mismatch")
    if require_string_evidence(first_health["privilege"], "native SCM first_health privilege") != "non-admin":
        raise RuntimeError("native SCM first health privilege mismatch")
    if require_string_evidence(first_health["listener_scope"], "native SCM first_health listener_scope") != "loopback-only":
        raise RuntimeError("native SCM first health listener scope mismatch")
    first_boot_id = require_string_evidence(first_health["boot_id"], "native SCM first_health boot_id")

    second_health = _require_object(payload["second_health"], "native SCM second_health")
    _require_exact_keys(
        second_health,
        frozenset({"boot_id", "service_identity", "privilege", "listener_scope"}),
        "native SCM second_health",
    )
    second_boot_id = require_string_evidence(second_health["boot_id"], "native SCM second_health boot_id")
    if second_boot_id == first_boot_id:
        raise RuntimeError("native SCM boot id did not change across restart")
    if require_string_evidence(second_health["service_identity"], "native SCM second_health service_identity") != r"NT SERVICE\HMSAgent":
        raise RuntimeError("native SCM second health service identity mismatch")
    if require_string_evidence(second_health["privilege"], "native SCM second_health privilege") != "non-admin":
        raise RuntimeError("native SCM second health privilege mismatch")
    if require_string_evidence(second_health["listener_scope"], "native SCM second_health listener_scope") != "loopback-only":
        raise RuntimeError("native SCM second health listener scope mismatch")

    if (
        require_string_evidence(payload["transport_target"], "native SCM transport_target")
        != "loopback-closed-port-retry-only"
    ):
        raise RuntimeError("native SCM transport target mismatch")
    require_false_evidence(
        payload["full_bridge_command_flow_proven"],
        "native SCM full_bridge_command_flow_proven",
    )
    require_false_evidence(
        payload["hyperv_guest_proven"], "native SCM hyperv_guest_proven"
    )
